...
A Stan model for this can be found in the [dist.stan](https://github.com/jmmorales/distance/raw/master/dist.stan) file
...
op = par(mfrow=c(1,2))
plot(density(samples$D), main = "")
abline(v = n/(2*B*L))
plot(density(samples$sigma), main = "")
abline(v=sigma)
par(op)
```
...
Z <- matrix(c(0.1, 0.5, 0.4, 0.5, -0.5, 0), (n.t + 1), n.p)
...
plot(scale(dgrass), Theta[,1], ylab = "", xlab = "", main = "intercept")
plot(scale(dgrass), Theta[,2], ylab = "", xlab = "", main = "slope")
...
```{r}
n_sites <- 35 # sampling units
# sample level predictor (environmental covariate)
x <- rnorm(n_sites)
B = 5 # max distance

# species indicator
j <- rep(1:n.s, each = n_sites) 
site <- rep(1:n_sites, n.s)
x.s <- rep(x, n.s)
...
stan_dat <- list(
  ...
  jj = c(data[,2], rep(1:n.s, each = nzj))
)

pars <- c("psi", "rho", "theta")

fit <- stan(file = 'jsd_cov.stan',
            data = stan_dat,
            #pars = pars,
            iter = 1000,
            thin = 1,
...
```{r}
bs <- fit_summary[grepl("theta", rownames(fit_summary)),]
